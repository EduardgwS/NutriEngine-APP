...
// ── Catálogo de receitas por objetivo ─────────────────────────────────────────

val receitasPorObjetivo: Map<Objetivo, List<RecomendacaoReceita>> = mapOf(
 ...
 *  assim a recomendação muda diariamente de forma determinística. */
fun escolherReceitaDoDia(objetivo: Objetivo): RecomendacaoReceita? {
    val lista  = receitasPorObjetivo[objetivo] ?: return null
    val indice = java.time.LocalDate.now().dayOfYear % lista.size
    return lista[indice]
}